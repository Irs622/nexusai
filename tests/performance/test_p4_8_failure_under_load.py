...
async def test_failure_under_load_stale_worker_token_rejection() -> None:
    ...
    try:
        ...
        lease_a = await coord.acquire_execution_lease(
            "exec-fail-1", "sess-fail-1", w_a, ttl_seconds=0.1
        )
        ...
        ), "Tool execution call_count MUST remain strictly 0 during stale worker failure injection!"
