...
def test_dependency_resolver_topological_sort():
    ...
    plugin_a = PluginManifest(id="plugin_a", name="A", version="1.0.0", entrypoint="mod:Class", dependencies=["plugin_b"])
    plugin_c = PluginManifest(id="plugin_c", name="C", version="1.0.0", entrypoint="mod:Class", dependencies=["plugin_a"])
...
def test_dependency_resolver_missing_dependency():
    resolver = DependencyResolver()
    plugin_a = PluginManifest(id="plugin_a", name="A", version="1.0.0", entrypoint="mod:Class", dependencies=["missing_b"])
...
def test_dependency_resolver_circular_dependency():
    resolver = DependencyResolver()
    plugin_a = PluginManifest(id="plugin_a", name="A", version="1.0.0", entrypoint="mod:Class", dependencies=["plugin_b"])
    plugin_b = PluginManifest(id="plugin_b", name="B", version="1.0.0", entrypoint="mod:Class", dependencies=["plugin_a"])
