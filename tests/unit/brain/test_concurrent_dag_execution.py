...
class ConcurrentSpyToolPort(IToolPort):
    ...
    def __init__(
        self, delay_map: dict[str, float] | None = None, fail_tools: set[str] | None = None
    ) -> None:
...
async def test_A_sequential_dag_correctness() -> None:
    ...
    nodes = {
        1: PlanGraphNode(
            step=PlanStep(step_id=1, title="Step 1", tool_name="tool_a"), dependencies=()
        ),
        2: PlanGraphNode(
            step=PlanStep(step_id=2, title="Step 2", tool_name="tool_b"), dependencies=(1,)
        ),
        3: PlanGraphNode(
            step=PlanStep(step_id=3, title="Step 3", tool_name="tool_c"), dependencies=(2,)
        ),
    }
...
async def test_B_independent_nodes_run_concurrently() -> None:
    ...
    nodes = {
        ...
        2: PlanGraphNode(
            step=PlanStep(step_id=2, title="Step 2", tool_name="tool_b"), dependencies=()
        ),
        3: PlanGraphNode(
            step=PlanStep(step_id=3, title="Step 3", tool_name="tool_c"), dependencies=()
        ),
    }
...
async def test_C_and_D_diamond_and_branching_dag() -> None:
    ...
    engine = PlanGraphExecutionEngine(max_concurrency=4)
    spy = ConcurrentSpyToolPort(
        delay_map={"tool_a": 0.05, "tool_b": 0.1, "tool_c": 0.1, "tool_d": 0.05}
    )
    ...
    nodes = {
        ...
        4: PlanGraphNode(
            step=PlanStep(step_id=4, title="Step D", tool_name="tool_d"), dependencies=(2, 3)
        ),
    }
    plan_graph = PlanGraph(nodes=nodes)
    engine.planner.plan = lambda ctx, session_id="": (plan_graph, MagicMock())  # type: ignore[assignment]
...
async def test_E_concurrency_limit_enforcement() -> None:
    ...
    spy = ConcurrentSpyToolPort(
        delay_map={"tool_a": 0.1, "tool_b": 0.1, "tool_c": 0.1, "tool_d": 0.1}
    )
    ...
    nodes = {
        ...
        4: PlanGraphNode(
            step=PlanStep(step_id=4, title="Step 4", tool_name="tool_d"), dependencies=()
        ),
    }
    plan_graph = PlanGraph(nodes=nodes)
    engine.planner.plan = lambda ctx, session_id="": (plan_graph, MagicMock())  # type: ignore[assignment]
    ...
    assert (
        spy.peak_concurrency <= 2
    ), f"Peak concurrency exceeded limit 2! Got {spy.peak_concurrency}"
...
async def test_F_and_G_non_sequential_and_string_node_ids() -> None:
    ...
    nodes: dict[Any, PlanGraphNode] = {
        "zeta": PlanGraphNode(
            step=PlanStep(step_id=1, title="Zeta", tool_name="tool_a"), dependencies=()
        ),
        "alpha": PlanGraphNode(
            step=PlanStep(step_id=2, title="Alpha", tool_name="tool_b"), dependencies=("zeta",)
        ),
        "beta": PlanGraphNode(
            step=PlanStep(step_id=3, title="Beta", tool_name="tool_c"), dependencies=("alpha",)
        ),
    }
    plan_graph = PlanGraph(nodes=nodes)
    engine.planner.plan = lambda ctx, session_id="": (plan_graph, MagicMock())  # type: ignore[assignment]
...
async def test_H_and_I_dependency_failure_propagation_and_unrelated_branch() -> None:
    ...
    assert (
        graph.nodes[4].step.status == StepStatus.CANCELLED
        or graph.nodes[4].step.status == StepStatus.PENDING
    )
...
async def test_N_deterministic_ready_node_scheduling() -> None:
    ...
    nodes = {
        "c_node": PlanGraphNode(
            step=PlanStep(step_id="c_node", title="C", tool_name="tool_c"), dependencies=()
        ),
        "a_node": PlanGraphNode(
            step=PlanStep(step_id="a_node", title="A", tool_name="tool_a"), dependencies=()
        ),
        "b_node": PlanGraphNode(
            step=PlanStep(step_id="b_node", title="B", tool_name="tool_b"), dependencies=()
        ),
    }
    plan_graph = PlanGraph(nodes=nodes)
    engine.planner.plan = lambda ctx, session_id="": (plan_graph, MagicMock())  # type: ignore[assignment]
