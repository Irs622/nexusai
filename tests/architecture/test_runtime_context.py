...
def test_execution_context_field_budgets():
    ...
    assert len(dataclasses.fields(CancellationContext)) <= 2, "CancellationContext exceeds 2 field limit!"
...
def test_working_memory_not_in_execution_context():
    ...
    assert "working_memory" not in telemetry_fields, "WorkingMemory must NOT be in TelemetryContext!"
    ...
    assert "WorkingMemory" not in src, "ExecutionContext source code must NOT reference WorkingMemory!"
