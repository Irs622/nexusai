...
async def test_fencing_token_blocks_stale_worker_side_effect() -> None:
    ...
    try:
        ...
        lease_a = await coord.acquire_execution_lease(
            "exec-fence-1", "sess-fence-1", w_a, ttl_seconds=0.1
        )
        ...
        await coord.recover_expired_execution_lease("exec-fence-1", w_b, ttl_seconds=10.0)

        # Worker A attempts tool execution using obsolete token_a (1)
        try:
            await coord.validate_lease_and_fencing_token(
                "exec-fence-1", w_a.worker_id, expected_token=token_a
            )
