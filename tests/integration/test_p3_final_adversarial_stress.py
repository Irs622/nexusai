...
async def test_p3_final_adversarial_agent_stress() -> None:
    ...
    # Register tools
    t1 = ToolMetadata(
        "terminal", "Terminal", "1.0.0", "Terminal", frozenset({ToolCapability.PROCESS_EXEC})
    )
    t2 = ToolMetadata(
        "file_reader", "FileReader", "1.0.0", "FileReader", frozenset({ToolCapability.FILE_READ})
    )
    ...
    gov = GovernanceEngine(
        global_budget=ResourceBudget(
            max_concurrent_tasks=10, max_subprocesses=15, max_tool_invocations=300
        ),
        telemetry=telemetry,
    )
    engine = PlanGraphExecutionEngine(governance=gov, telemetry=telemetry)
    loop = AgentLoop(
        execution_engine=engine, tool_registry=registry, memory_store=mem_store, telemetry=telemetry
    )
    ...
    async def run_agent_loop_worker(w_id: int) -> None:
        req = AgentRequest(
            session_id=f"sess-p3-final-{w_id}", user_prompt=f"Final stress task {w_id}"
        )
    ...
    async def approval_worker(a_id: int) -> None:
        ...
        req = HumanApprovalRequest(
            f"app-final-{a_id}", binding, RiskLevel.HIGH, f"Execute task {a_id}"
        )
        await approval_engine.request_approval(req)

        dec = HumanApprovalDecision(
            f"app-final-{a_id}", ApprovalStatus.APPROVED, "operator@co.com", "Approved"
        )
    ...
    ), "Zero resource leak invariant must hold after teardown!"
