...
async def test_p4_2_adversarial_real_tool_stress() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        NetworkTool(allowed_hosts={"api.github.com"})
        gov = GovernanceEngine(
            global_budget=ResourceBudget(
                max_concurrent_tasks=20, max_subprocesses=25, max_tool_invocations=100
            )
        )

        # 1. 20 Concurrent Filesystem operations
        async def fs_worker(w_id: int) -> None:
            res_gov = await gov.authorize(
            ...
            w_req = ToolExecutionRequest(
                ...
                "filesystem_tool",
                {"action": "write_file", "path": f"f_{w_id}.txt", "content": f"Data {w_id}"},
            )
        ...
        async def proc_worker(w_id: int) -> None:
            ...
            p_req = ToolExecutionRequest(
                ...
                "process_tool",
                {"argv": [sys.executable, "-c", f"print('Stress worker {w_id}')"]},
            )
        ...
        workers = [asyncio.create_task(fs_worker(i)) for i in range(20)] + [
            asyncio.create_task(proc_worker(j)) for j in range(15)
        ]
