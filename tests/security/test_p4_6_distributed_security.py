...
async def test_security_stale_fencing_token_execution_blocked() -> None:
    ...
    try:
        ...
        lease_a = await coord.acquire_execution_lease(
            "exec-sec-1", "sess-sec-1", w_a, ttl_seconds=0.1
        )
        ...
        with pytest.raises((FencingTokenError, StaleWorkerError)):
            await coord.validate_lease_and_fencing_token(
                "exec-sec-1", w_a.worker_id, expected_token=token_a
            )
...
async def test_security_stale_worker_cannot_release_current_lease() -> None:
    ...
    try:
        ...
        await coord.acquire_execution_lease("exec-sec-2", "sess-sec-2", w_a, ttl_seconds=0.1)
        await asyncio.sleep(0.15)
