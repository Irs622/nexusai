...
async def test_security_p5_2_fencing_token_stale_worker_rejection() -> None:
    ...
    for coord in (pg_coord, redis_coord):
        ...
        lease_a = await coord.acquire_execution_lease("exec-p5-sec-1", "sess-p5-sec-1", w_a, ttl_seconds=0.1)
        ...
        lease_b = await coord.recover_expired_execution_lease("exec-p5-sec-1", w_b, ttl_seconds=10.0)
        ...
        with pytest.raises((FencingTokenError, StaleWorkerError)):
            await coord.validate_lease_and_fencing_token("exec-p5-sec-1", w_a.worker_id, expected_token=token_a)
