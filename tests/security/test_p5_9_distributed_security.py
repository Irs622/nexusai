...
async def test_security_multi_node_stale_worker_side_effect_strictly_zero() -> None:
    ...
    lease_a = await coord.acquire_execution_lease(
        "exec-p59-sec-1", "sess-p59-sec-1", w_node_a, ttl_seconds=0.1
    )
    ...
    lease_b = await coord.recover_expired_execution_lease(
        "exec-p59-sec-1", w_node_b, ttl_seconds=10.0
    )
    ...
    try:
        if await coord.validate_lease_and_fencing_token(
            "exec-p59-sec-1", w_node_a.worker_id, expected_token=token_a
        ):
            await tool_port.execute(
                pytest.importorskip("nexusai.brain.ports.tool_port").ToolExecutionRequest(
                    "exec-p59-sec-1", "process_tool", ()
                )
            )
    ...
    ), "Rejected stale worker tool execution call_count MUST BE STRICTLY 0!"
...
async def test_security_recovery_epoch_invalidation_blocks_stale_worker_side_effects() -> None:
    ...
    await coord.acquire_execution_lease(
        "exec-p59-sec-2", "sess-p59-sec-2", w_node_a, ttl_seconds=10.0
    )

    # Disaster recovery occurs -> epoch += 1
    rec_mgr = DisasterRecoveryManager(backup_prov, verifier, coord, audit_store)
    await backup_prov.create_backup("bak-p59-epoch")
    ...
    assert (
        tool_port.call_count == 0
    ), "Tool execution call_count MUST BE STRICTLY 0 under old recovery epoch!"
...
async def test_security_dual_lease_acquisition_exactly_one_winner() -> None:
    ...
    async def acquire_attempt(w: WorkerIdentity) -> bool:
        try:
            lease = await coord.acquire_execution_lease(
                "exec-p59-race", "sess-p59-race", w, ttl_seconds=10.0
            )
    ...
    ), "EXACTLY ONE worker node MUST win dual/triple lease acquisition race!"
