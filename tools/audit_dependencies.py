        ...
        for item in imports:
            ...
            if pkg == "providers":
                for forbidden in FORBIDDEN_PACKAGES:
                    if f"nexusai.{forbidden}" in full_imp or module.startswith(
                        f"nexusai.{forbidden}"
                    ):
                        violations.append(
                            (
                                ...
                                lineno,
                                full_imp,
                                f"providers package MUST NOT import nexusai.{forbidden}",
                            )
                        )
            ...
            if pkg == "runtime":
                for concrete_mod in CONCRETE_MODULES:
                    if f"nexusai.providers.{concrete_mod}" in full_imp or module.startswith(
                        f"nexusai.providers.{concrete_mod}"
                    ):
                        violations.append(
                            (
                                "A002",
                                ...
                                lineno,
                                full_imp,
                                f"runtime MUST NOT import concrete provider module '{concrete_mod}'",
                            )
                        )
                if imported_symbol in CONCRETE_PROVIDERS:
                    violations.append(
                        (
                            ...
                            lineno,
                            full_imp,
                            f"runtime MUST NOT import concrete provider class '{imported_symbol}'",
                        )
                    )
                    ...
                    if f"nexusai.providers.{concrete_mod}" in full_imp or module.startswith(
                        ...
                        violations.append(
                            (
                                ...
                                lineno,
                                full_imp,
                                f"brain MUST NOT import concrete provider module '{concrete_mod}'",
                            )
                        )
                if imported_symbol in CONCRETE_PROVIDERS:
                    violations.append(
                        (
                            ...
                            lineno,
                            full_imp,
                            f"brain MUST NOT import concrete provider class '{imported_symbol}'",
                        )
                    )
                ...
                if "nexusai.providers" in full_imp or module.startswith("nexusai.providers"):
                    violations.append(
                        (
                            ...
                            lineno,
                            full_imp,
                            "memory MUST NOT import nexusai.providers",
                        )
                    )
                    ...
                    if f"nexusai.providers.{concrete_mod}" in full_imp or module.startswith(
                        ...
                        violations.append(
                            (
                                ...
                                lineno,
                                full_imp,
                                f"workflow MUST NOT import concrete provider module '{concrete_mod}'",
                            )
                        )
                if imported_symbol in CONCRETE_PROVIDERS:
                    violations.append(
                        (
                            ...
                            lineno,
                            full_imp,
                            f"workflow MUST NOT import concrete provider class '{imported_symbol}'",
                        )
                    )
                    ...
                    if f"nexusai.providers.{concrete_mod}" in full_imp or module.startswith(
                        ...
                        violations.append(
                            (
                                ...
                                lineno,
                                full_imp,
                                f"security MUST NOT import concrete provider module '{concrete_mod}'",
                            )
                        )
                if imported_symbol in CONCRETE_PROVIDERS:
                    violations.append(
                        (
                            ...
                            lineno,
                            full_imp,
                            f"security MUST NOT import concrete provider class '{imported_symbol}'",
                        )
                    )

    print(
        f"Audit completed across {len(all_files)} files. Total Violations Found: {len(violations)}\n"
    )
    if violations:
        for v in violations:
