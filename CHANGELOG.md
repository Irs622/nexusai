...
- `security(token)`: **NEX-003 (Approval token single-use & distributed nonce store)**: Implement `IDistributedNonceStore` interface with `RedisNonceStore`, `PostgresNonceStore`, and in-memory implementations. Enforce non-default `NEXUSAI_APPROVAL_SECRET` and distributed nonce store when running in production (`env == "production"`). Fix ambient identity binding so `user_id` is resolved from caller context when creating and validating approval tokens.
- `security(rbac)`: **NEX-004 (Fail-closed authorization on missing/anonymous identity)**: Refactor `evaluate_permission` in `SecurityGuard` and `check_permission` in `RbacEngine` to fail-closed (`AuthenticationError` -> `DENY`) when caller identity is missing or unauthenticated, eliminating legacy permissive fallback to `Role.ADMIN`.
- `security(api)`: **NEX-005 (Cross-tenant SSE event leakage)**: Bind SSE stream connections (`/api/v1/events/stream`) to verified request `tenant_id` and restrict broadcast events strictly to listeners belonging to the originating tenant.
- `security(tools)`: **NEX-006 (Tenant-isolated tool execution)**: Propagate `tenant_id` from `BrainCoordinator` into `ExecuteToolCommand` to enforce tenant workspace boundaries and audit trails on tool execution.
- `security(dag)`: **NEX-007 (DAG execution idempotency race condition)**: Maintain `RUNNING` idempotency status during task execution; transition to `SUCCEEDED` or `FAILED_TERMINAL` only upon actual execution completion in `_run_dag_durable()`, rejecting duplicate concurrent submissions with HTTP 409 Conflict.
- `security(rbac)`: **NEX-008 (Unauthenticated MCP endpoints)**: Enforce explicit RBAC matrix on MCP server inspection endpoints: require `Role.OPERATOR` or `Role.ADMIN` for `GET /api/v1/mcp/servers/{server_id}/ping` and require `Role.ADMIN` for `POST /api/v1/mcp/servers/{server_id}/reload`.
- `security(planner)`: **NEX-009 (Unrestricted tool DAG generation)**: Constrain DAG execution plans to selected candidate tools and their explicit topological prerequisites from `CapabilityGraph`, preventing unauthorized or irrelevant capability invocation in generated plans.
- `security(runtime)`: **NEX-010 (Ephemeral in-memory coordinator topology)**: Add startup topology validation in `create_app()` forbidding volatile in-memory databases (`:memory:`) for execution and coordinator storage in production or multi-replica configurations; configure persistent volume mounts and Redis in Helm production values.
- `security(supply-chain)`: **NEX-011 (Unpinned container dependencies)**: Generate fully pinned `requirements.lock` and update `Dockerfile` to install dependencies strictly from `requirements.lock` with `--no-deps`.
- `security(validator)`: **NEX-012 (OutputValidator cross-request state pollution)**: Instantiate request-scoped `OutputValidator` per `process_user_input()` call in `BrainCoordinator`, preventing sensitive file access history from leaking across concurrent or sequential multi-tenant requests.
- `test(security)`: Implement automated regression test suite (`tests/unit/security/test_audit_remediation.py`) verifying all 12 audit findings (12/12 passing).
