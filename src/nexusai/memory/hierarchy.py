...
class MemoryHierarchy:
    ...
    def __init__(
        self, sqlite_memory: SQLiteMemory, vector_store: Optional[VectorKnowledgeBase] = None
    ) -> None:
    ...
    async def compact_history(self, session_id: str, max_turns: int = 10) -> str:
        ...
        summary_text = f"Compacted Summary of {len(old_turns)} turns: " + " | ".join(
            [m["content"] for m in old_turns[:3]]
        )

        if self.vector_store is not None:
            await self.vector_store.store_memory(
                summary_text, metadata={"session_id": session_id, "type": "compacted_summary"}
            )

        return summary_text

    async def query_relevant_context(
        self, session_id: str, query: str, history_limit: int = 5
    ) -> Dict[str, Any]:
