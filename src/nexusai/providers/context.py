...
class CancellationToken:
    ...
    def cancel(self, reason: str = "Execution cancelled by user or system") -> None:
        """Trigger cancellation for this token and propagate to child tokens."""
        if self._is_cancelled:
            return
    ...
    def create_child(self) -> CancellationToken:
        """Create a child cancellation token linked to this parent."""
        return CancellationToken(parent=self)

    def throw_if_cancelled(self) -> None:
        """Raise ProviderTimeoutError if cancellation signal is active.

        Raises:
            ProviderTimeoutError: If token is cancelled.
        """
...
class Deadline:
    ...
    def from_seconds(cls, seconds: float, clock: Optional[Clock] = None) -> Deadline:
        ...
        deadline_at = datetime.fromtimestamp(c.time() + seconds, tz=timezone.utc)
    ...
    def throw_if_expired(self) -> None:
        """Raise ProviderTimeoutError if deadline has passed.

        Raises:
            ProviderTimeoutError: If deadline is expired.
        """
        if self.is_expired():
